...
--- Populates Framework.Server with QBCore-specific wrappers.
--- Required by libs/shared/framework.lua on the server when QBCore is detected.

local QBCore = exports['qb-core']:GetCoreObject()

--- Get the QBCore player object for a source.
---@param source number
---@return table|nil
Framework.Server.getPlayer = function(source)
    return QBCore.Functions.GetPlayer(source)
...
---@param source number
---@param account string  'cash' | 'bank' | 'crypto'
---@return number
Framework.Server.getMoney = function(source, account)
    local player = Framework.Server.getPlayer(source)
    if not player then return 0 end
